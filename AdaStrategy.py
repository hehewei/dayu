...
########################################################################
class AdaStrategy(CtaTemplate):
    className = 'AdaStrategy'
    author = 'HeHengWei'
    version = '1.1.11'
    ...
    trailingPercent = 2
    atrMultipler = 3
    MacdFastPeriodLen = 12
    MacdSlowPeriodLen = 26
    MacdSignalPeriodLen = 9
    # 策略变量
    # 策略变量
    ...
    def __init__(self, ctaEngine, setting):
        super(AdaStrategy, self).__init__(ctaEngine, setting)
    ...
    def onInit(self):
        ...
        #       将同时生成 self.bg60Dict 和 self.am60Dict ,字典的key是品种名,
        #       用于生成 on5MinBar 需要的 Bar 和计算指标信号用的 bar array，可在 on5MinBar() 获取到
        self.generateBarDict(self.onBar)
        self.generateBarDict(self.onBar, 60, self.on60MinBar)
    ...
    def on60MinBar(self, bar):
        """收到Bar推送（必须由用户继承实现）"""
        symbol = bar.vtSymbol
        self.am60Dict[symbol].updateBar(bar)  # 需要将 15MinBar 数据同时推给 15MinBar 的array字典去保存，用于talib计算
        am60 = self.am60Dict[symbol]

        if not am60.inited:
            return
        atr = ta.ATR(am60.high, am60.low, am60.close, 10)
        dif,dea,macd = ta.MACD(am60.close,self.MacdFastPeriodLen,self.MacdSlowPeriodLen,self.MacdSignalPeriodLen)
            # 计算策略需要的信号-------------------------------------------------
        direction = am60.close[-1] - am60.close[-21]
        volatility = sum(abs(am60.close[-20:] - am60.close[-21:-1]))
        ER = direction / volatility
        ER = round(ER, 2)

        fastest = 2 / (self.fastN + 1)
        slowest = 2 / (self.slowN + 1)
        c = (ER * (fastest - slowest) + slowest) ** 2

        if not self.AMAlist:
            self.AMAlist.append(am60.close[-1])
        else:
            self.AMAlist.append(self.AMAlist[-1] + c * (am60.close[-1] - self.AMAlist[-1]))

        if len(self.AMAlist) > 80:
            del self.AMAlist[0]

        filter_ = ta.STDDEV(np.array(self.AMAlist), self.FilterLength, 1) * self.FilterSet

        if len(filter_) < self.FilterLength:
            return
        # 构建进出场逻辑-------------------------------------------------
        # 如果金叉时手头没有多头持仓
        if (self.AMAlist[-1] - self.AMAlist[-2] >= filter_[-1]):
            ...
            if self.posDict[symbol + "_LONG"] == 0 and self.stopLossControl == 0:
                self.buy(symbol,  # 下单交易品种
                         ...
                         # 价格类型：[PRICETYPE_LIMITPRICE,PRICETYPE_MARKETPRICE,PRICETYPE_FAK,PRICETYPE_FOK]
                         levelRate=10)  # 保证金交易可填杠杆参数，默认levelRate = 0

            # 如果有空头持仓，则先平空，再做多
        if (dif[-1] < dea[-1]) and (dif[-2] > dea[-2]):  # parameter6
            if self.posDict[symbol + "_LONG"] > 0:
                self.cancelAll()
                self.sell(symbol, bar.close * 0.99, self.posDict[symbol + "_LONG"], levelRate=10)
        # 如果金叉时手头没有空头持仓
        if (self.AMAlist[-2] - self.AMAlist[-1] >= filter_[-1]):
            if self.stopLossControl == 1:
                self.stopLossControl = 0
        ...
        # 持有多头仓位
            ...
            self.intraTradeHighDict[symbol] = max(self.intraTradeHighDict[symbol], bar.high)
            self.longStop = max(self.longStop,
                                        self.intraTradeHighDict[symbol] - self.atrMultipler * atr[-1])
        ...
        # 持有空头仓位
            ...
            self.intraTradeLowDict[symbol] = min(self.intraTradeLowDict[symbol], bar.low)
            self.shortStop = min(self.shortStop,
                                         self.intraTradeLowDict[symbol] + self.atrMultipler * atr[-1])
    ...
    def onOrder(self, order):
        ...
        if order.status == STATUS_UNKNOWN:
            self.mail(u'出现未知订单，需要策略师外部干预,ID:%s, symbol:%s,direction:%s,offset:%s'
                      % (order.vtOrderID, order.vtSymbol, order.direction,
                         order.offset))  # 在交易通讯异常时，系统返回未知状态的order对象给策略，需要额外措施处理
...
engine.setEndDate('20181128 12:00') # 设置回测用的数据终止日期
...
engine.initStrategy(AdaStrategy, parameterDict)    # 创建策略对象
